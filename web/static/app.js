...
function play() {
  ...
  const tick = async () => {
    ...
    await step();
    const wait = 280 - Number($("speed").value);
    state.timer = setTimeout(tick, Math.max(40, wait));
  };
  tick();
}
...
function onMapClick(ev) {
  ...
  post("/api/goal", { y, x }).then(apply);
}

async function bootScenes() {
  const data = await (await fetch("/api/scenes")).json();
  $("scenes").innerHTML = data.stems
    .map((s, i) => `<option value="${i}">${String(i).padStart(3, "0")}  ${s}</option>`)
    .join("");
}
...
bootScenes()
  .then(start)
  .catch((err) => {
    $("wait").textContent = "could not start";
